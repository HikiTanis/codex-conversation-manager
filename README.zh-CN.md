...
- 删除时会先调用 Codex 官方 `thread/delete` 接口更新任务目录；只有官方接口确认成功后，才处理会话文件、本地 SQLite 线程索引、Codex 桌面状态及准确命中的桌面任务列表缓存。官方接口不可用时会保留原会话，不会继续形成新的半删除记录。
...
- 如果会话文件和线程索引已经删除，但旧的 Codex 桌面响应缓存仍让条目留在侧边栏，工具会在 Codex 完全退出后按已确认删除的 Thread ID 精确核对并失效对应任务列表缓存；不会清除登录信息、正常任务或项目文件。
...
4. 删除或恢复前完全退出 Codex；如果旧版本已经留下点开报错的侧边栏条目，先在 Codex 中点开一次让错误写入日志，再完全退出 Codex，打开本工具点击“刷新”，核对提示中的标题和 Thread ID 后确认清理。如果会话与索引此前已经删除但侧边栏仍显示，刷新还会补清理准确命中的桌面任务列表缓存，并提示何时可以重新打开 Codex。
