...
- 删除时会先调用 Codex 官方 `thread/delete` 接口更新任务目录；只有官方接口确认成功后，才处理会话文件、本地 SQLite 线程索引和 Codex 桌面状态。官方接口不可用时会保留原会话，不会继续形成新的半删除记录。
...
4. 删除或恢复前完全退出 Codex；如果旧版本已经留下点开报错的侧边栏条目，先在 Codex 中点开一次让错误写入日志，再完全退出 Codex，打开本工具点击“刷新”，核对提示中的标题和 Thread ID 后确认清理。
